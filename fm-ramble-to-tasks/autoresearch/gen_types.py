...
Rule: a config is weight-level iff its SplitConfig sets `adapter:`. The training
...
def parse_configs(text):
    """label -> type, by scanning each `"label": SplitConfig(...)` entry until
    the next entry. An entry is weight-level iff it contains an `adapter:` path."""
    out, label, buf = {}, None, []
    label_re = re.compile(r'^\s*"([A-Za-z0-9_]+)"\s*:\s*SplitConfig\(')
...
def main():
    ...
    # Fallback for any logged label missing from Configs.swift so the table is never blank.
    if RUNS.exists():

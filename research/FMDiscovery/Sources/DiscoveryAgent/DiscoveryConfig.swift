...
public struct DiscoveryConfig: Sendable {
    public enum Topology: String, Sendable {
        ...
        case adapterOverCountDedupRepair // exp046 over-count(8)-then-dedup base + the exp042 zero-risk deterministic phrasing repair stacked on the FINAL 7 (third→second person + high-precision compound de-splitting) — restores the atomicity/naturalness the count=8 perturbation cost while keeping the champion-native nonRed-4 gain
        case adapterOverCountDedup9 // exp048 GENERALIZES exp046 to TWO spares: schema .count(9) → champion writes two extra native questions, then ITERATIVELY drop the later member of each redundant pair (binary same-info OR cos≥0.75 OR Jaccard≥0.5) until 7 remain (tail-drop if no redundancy left). Targets the multi-redundant cases (gp/team_offsite/household_budget) that waste 2-3 slots and which exp046's single drop could only half-fix
        case adapterCorpusOverCountDedup // exp049 STACKS the two strongest orthogonal add-ons: the exp040 corpus-RAG few-shot demos (coverage/specificity prior in the system prompt, tied 0.405) AND the exp046 over-count(8)-then-dedup mechanism (non-redundancy fix, 0.430). One greedy native call with the corpus-augmented system prompt and schema .count(8); then deterministically drop the redundant later member of the first redundant pair (binary same-info OR cos≥0.75 OR Jaccard≥0.5). Bet: corpus demos lift coverage/spec while the native spare + dedup lift non-redundancy — gains on different rubric axes should stack above 0.430. All native voice (no foreign-source specificity tax); gold-leak ban respected via corpus near-dup ceiling + LOO
        ...
        case adapterOverCountDistinctDrop // exp051: same over-count(8) + high-recall redundancy union as champion exp046, but when a redundant pair is found, drop the LESS-DISTINCTIVE member (higher aggregate cos/Jaccard similarity to the other six) instead of the later one — keeping whichever member adds more unique coverage to the final 7. exp046 mechanically drops the LATER slot, which can discard a distinctive late-surfacing unknown and keep a generic early duplicate; choosing by distinctiveness directly targets the coverage axis while staying fully native-voice and deterministic. Floor identical to exp046 (drop the 8th when no redundancy)
        case adapterConditionalSpare // exp052: make the champion exp046 over-count CONDITIONAL. Take the PURE greedy count=7 champion draft (atom-5 native voice), detect redundancy with the same high-recall union (Jaccard/cos/binary same-info); if NO redundant pair, return the pure draft VERBATIM (atom-5 floor, avoiding the count=8 perturbation exp046 applies to every case); ONLY if a redundant pair exists, fall back to the exact exp046 count=8-then-dedup native-spare fix. Weakly dominates exp046: recovers the atomicity its blanket count=8 cost on clean cases while keeping the nonRed-4 gain on redundant ones
        case adapterOverCountHiRecall // exp053: EXACT exp046 over-count(8)-then-dedup, but the ONLY change is the binary semantic redundancy JUDGMENT — the documented binding constraint. exp046's zero-shot single-order "essentially the same information" check under-fires on the deep low-overlap paraphrase pairs that survive in the multi-redundant cases (team_offsite venue×2, gp location overlap, household_budget fixed/variable, find_therapist Q1≈Q2≈Q5). Replace it with a RECALL-TILTED, 2-shot (genuine-paraphrase + clearly-distinct exemplars), BOTH-ORDERS-OR'd check that leans true when unsure. Jaccard (0.5) and cos (0.75) thresholds, the count=8 draw, the first-redundant-pair drop-later-member rule, and the no-redundancy 8th-drop floor are ALL identical to exp046 — a clean single-variable A/B isolating detector recall
        case adapterOverCountGivensDedup // exp050: extend the champion exp046 over-count(8) base to attack a SECOND judge-confirmed waste mode it ignores — questions that RE-ASK a fact the task statement already provides (apartment_move "which city" when task says "across the city"; buy_used_car "new or used?" when task says "used"; dinner_party location-already-stated). Same single greedy native count=8 call (intact native set, native spare). Then a lever-F LOCAL BINARY reading check on the adapter ("does the task text ALREADY state this fact?" — reading comprehension, not judgment, the reliable small-model signal per exp037) flags any slot that merely restates a given; drop the FIRST flagged given-restatement (keeping 7 native), ELSE fall back to exp046's first-redundant-pair drop, ELSE drop the 8th (≈champion first-7 floor). Frees a confirmed-wasted slot for a genuine unknown; champion-priority, deterministic, intact native set preserved
        case adapterParallelDrawSpare // exp055: recover the atomicity-5 that champion exp046 (0.430) sacrificed. exp046 lifts nonRed 3→4 by sourcing ONE extra question from a .count(8) schema, but count=8 perturbs the native draw into un-truncatable noun-pair compounds ("breed or age", "current age and retirement age") that cap atomicity at 4 (the pure count=7 champion held atom 5). exp055 sources the redundancy-fixing spare WITHOUT the count=8 perturbation: PRIMARY draw is the pure greedy count=7 champion (atom-5 native voice); build the same exp054 high-recall redundancy graph (Jaccard≥0.5 OR cos≥0.75 OR binary same-info), drop the highest-degree node → 6 native questions; then draw a SECOND seeded count=7 from the SAME champion adapter and splice in its single question that is NOT redundant with the kept 6 and is MOST distinct (max min-cosine-distance) → a champion-native, atom-5, specific 7th. No redundancy → return the pure champion verbatim (atom-5 / 0.409 floor). The spare is same-adapter native voice (no count=8 atom tax, no foreign-donor spec tax of exp035), targeting atom-5 + nonRed-4 above exp046's atom-4 + nonRed-4
        case adapterCorpusCoverageSelect // exp056: EXACT exp046 over-count(8) greedy native draw (proven spec-4 sweet spot, no prompt perturbation), but replace exp046's INTERNAL redundancy-drop with an EXTERNAL corpus-coverage-grounded drop. exp046/051/054 all changed WHICH-of-8-to-drop using only INTERNAL signals (first-redundant-pair / distinctiveness / degree) and saturated at 0.42-0.43; exp034's internal facility-location (max internal spread) collapsed coverage to 2 (diversity surfaces niche off-task). The ONLY inference signal that EVER helped is the EXTERNAL corpus coverage prior (exp040 corpus RAG, 0.405) — but it was used in the GENERATION prompt, where exp049 showed it perturbs the count=8 sweet spot and CANCELS exp046's gain. exp056 moves that prior into the deterministic SELECTION step: retrieve the k nearest CORPUS task-sets (LOO, full bank, anti-leak jaccardCeiling), embed their questions as the gold-like coverage AXES, then drop the ONE of the 8 native candidates whose removal least reduces total corpus-axis coverage (sum over axes of max cosine to a kept question), tie-break dropping the more internally-redundant one. Keeps the 7 native questions that best MATCH the gold coverage distribution — fixing exp034's internal-diversity flaw (anchor externally, not on spread) and exp049's perturbation flaw (prior in selection, not generation) and exp035/044's specificity tax (native voice, no donor). Single greedy call, deterministic, champion floor when vectors unavailable.
        case adapterOverCountGlobalDedup // exp054: EXACT exp046 over-count(8) draw + EXACT high-recall union detector (Jaccard≥0.5 OR cos≥0.75 OR binary same-info) — the ONLY change is the DROP RULE. exp046 drops the later member of the FIRST redundant pair; exp048/051 documented that the binding constraint is the MULTI-redundant cases where ONE question is redundant with SEVERAL others (find_therapist Q1≈Q2≈Q5, team_offsite date×2+venue×2, gp location overlap) — a single first-pair drop only HALF-fixes them, leaving redundancy standing → nonRed capped at 4. exp054 builds the FULL redundancy graph over all 8, counts each question's redundancy DEGREE (how many others it duplicates), and drops the single highest-degree node — the one drop that clears the MOST redundancy (dropping Q1 in Q1≈Q2≈Q5 resolves two links at once). Tie on degree → drop the less-distinctive member (higher aggregate cos to the rest, exp051's rule); further tie → later index. No redundancy at all → drop the 8th (exp046 floor). Same single greedy call, same detector thresholds (no recall/precision change → avoids exp053's false-positive trap), native voice, deterministic. Single-variable A/B isolating first-pair vs global-degree drop selection.
    }

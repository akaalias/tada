...
export async function renderDetail(label, cell) {
  ...
  const blocks = scores.map((s, i) => {
    ...
    const maxN = cand ? cand.questions.length : 0;
    ...
    const fmQs = cand ? `<ol class="qs">${cand.questions.map((q, qi) =>
        `<li data-qn="${qi + 1}"${refd.has(qi + 1) ? ' class="q-flag"' : ''}>${esc(q.title)}</li>`).join('')}</ol>`
    ...
    return `<div class="case">
      <div class="case-head"><span class="case-task">${esc(s.input)}</span><span class="case-meta">${head}</span></div>
      <div class="cmp">

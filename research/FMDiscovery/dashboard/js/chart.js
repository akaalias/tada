...
export function drawChart(runs) {
  ...
  bestPts.forEach(([i, b], k) => { const x = X(i), y = Y(b); if (k === 0) { ctx.moveTo(x, y); } else { ctx.lineTo(X(i - 1), y); ctx.lineTo(x, y); } });
  ctx.stroke();
  ...
  runs.forEach((r, i) => {
    ...
    if (r.kept) { ctx.fillStyle = GREEN; ctx.font = '12px system-ui'; ctx.fillText(r.note ? r.note : r.label, x + 9, y - 9); }
  });
}
